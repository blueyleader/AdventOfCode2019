...
fn main() {
    ...
    let mut starting_pos = (25,25);
    for (key,val) in map.clone() {
        if(val==2){
            starting_pos = key;
        }
    }
    let mut sum: i64 = map.values().filter(|x| **x as i64==1).sum();
        ...
        for (key,val) in map.clone(){
            if(val==2){
                ...
                for m in moves {
                    ...
                    if *map2.get(&(key.0+m.0,key.1+m.1)).unwrap_or(&0) == 1{
                        map2.insert((key.0+m.0,key.1+m.1),2);
                    }
                }
            }
        }
        map = map2.clone();
        sum = map.values().filter(|x| **x as i64==1).sum();
        count+=1;
