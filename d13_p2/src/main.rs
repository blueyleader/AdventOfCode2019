...
fn main() {
    ...
    let mut map:Vec<Vec<i64>> = vec![vec![0;40];20];
        ...
        match opt {
            ...
            3 => { //input: input->p1
                ...
                for row in map.clone() {
                    for (col, x) in row.iter().zip(0..row.len()) {
                        print!("{}", if *col==0 { " ".to_string() } else if *col==4 { "O".to_string() } else { col.to_string() });
                        if *col == 4 as i64 {
                            ball=x;
                        } 
                        else if *col == 3 as i64 {
                            pad=x;
                        } 
                    }
                    print!("\n");
